    ...
    class Problem11 : IProblem {
        ...
        private long LargestProductInGrid(int[,] grid, int adjecent = 4) {
            if (grid.GetLength(0) != grid.GetLength(1)) {
                throw new ArgumentException("Supplied array is not a grid");
            }
            long result = 0;
            for (int i = 0; i < grid.GetLength(0); i++) {
                for (int j = 0; j < grid.GetLength(1); j++) {
                    ...
                    // Horizontal
                    if (j + adjecent < grid.GetLength(1)) {
                        long sum = 1;
                        for (int k = 0; k < adjecent; k++) {
                            sum *= grid[i, j + k];
                        }
                        if (sum > result) {
                            result = sum;
                    ...
                    // Vertical
                    if (i + adjecent < grid.GetLength(0)) {
                        long sum = 1;
                        for (int k = 0; k <adjecent; k++) {
                            sum *= grid[i + k, j];
                        }
                    ...
                    // Diagonally Left
                    if (j - adjecent >= 0 && i + adjecent < grid.GetLength(0)) {
                        long sum = 1;
                        for (int k = 0; k < adjecent; k++) {
                            sum *= grid[i + k, j - k];
                        }
                    ...
                    // Diagonally Right
                    if (j + adjecent < grid.GetLength(1) && i + adjecent < grid.GetLength(0)) {
                        long sum = 1;
                        for (int k = 0; k < adjecent; k++) {
                            sum *= grid[i + k, j + k];
                        }
